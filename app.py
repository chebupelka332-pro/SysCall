...
st.sidebar.info(
    "Измерьте RSSI на 1 метре, чтобы найти `A (Tx Power)`.")
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # --- ИЗМЕНЕНО: Используем 4 лучших маячка вместо 3 ---
        N_BEST_BEACONS = 4
        ...
        # --- ИЗМЕНЕНО: Проверяем, что у нас есть как минимум 4 маячка для расчета ---
        if len(beacons_for_calc) < 4:
